...
func (m Manager) exampleRestCall(c *gin.Context) {
	...
	lrsp := genericRsp.(ExampleWebResponse)
...
func (m Manager)exampleWorkerHandle (workerCtx *WorkerContext) {
	...
	var rsp ExampleWebResponse

	webRequst := workerCtx.workerRequest.Body.(ExampleWebRequest)

	rsp = ExampleWebResponse {
